...
core_data_files = os.listdir("nis_core_data_files")


def createcoretable(cur):
    i = 0
    for file in core_files:
        file_name = os.path.basename(file).split('_')
        table_name = 'NIS_' + file_name[2] + '_Core'
        with open(os.path.abspath(os.path.join('nis_core_files', file)), 'r') as f:
            data = f.readlines()
            ...
            while True:
                insert_core_data(sql_data, core_data_files[i])
                i += 1
                if i <= len(core_data_files):
                    break


def insert_core_data(sql_data, filename):
    table_name = 'NIS_' + filename.split('_')[1] + '_CORE'
    sql_column_name = [item[0] for item in sql_data]

    with open(os.path.abspath(os.path.join('nis_core_data_files', filename)), 'r') as f:
